...
<!--chapter:end:02-lm.protocol.Rmd-->
...
# allometry - unique allometries -> geo
fit.sz.ucomb <- procD.lm(shape ~ size * geo, 
                        data = gdf, 
                        print.progress = FALSE, 
                        iter = 9999)

# allometry - unique allometries -> time
fit.sz.ucomb <- procD.lm(shape ~ size * time, 
                        data = gdf, 
                        print.progress = FALSE, 
                        iter = 9999)
...
# unique shapes for trajectory analysis
fit.traj <- procD.lm(shape ~ geo * time,
                data = gdf,
                print.progress = FALSE,
                iter = 9999)
```
...
TA <- trajectory.analysis(fit.traj, 
                          groups = qdata$geo, 
                          traj.pts = qdata$time, 
                          print.progress = FALSE)
